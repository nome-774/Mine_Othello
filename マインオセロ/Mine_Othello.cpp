...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_  HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nShowCmd) {
    ...
    int SmallHandle = CreateFontToHandle(NULL, 12, 3);
    ...
    int sign;       // フラグの画像情報
    int effect[8];  // 爆破の画像情報
    int disc[8];    // オセロの石の画像情報

    /* 画像情報の代入 */
    back = LoadGraph("board.png");
    sign = LoadGraph("flag.png");
    LoadDivGraph("exp.png", 8, 2, 4, 48, 48, effect);
    ...
    while (!ProcessMessage()) {
        ...
        DrawStringToHandle(250, 13, "  CPU      枚", GetColor(255, 255, 255), DefaultHandle);
        ...
        DrawStringToHandle(44, 474, " Player    枚", GetColor(255, 255, 255), DefaultHandle);
        ...
        for (int y = 0; y < 8; y++) {
            for (int x = 0; x < 8; x++) {
                ...
                if (situation[y][x] == 1) {
                    ...
                    DrawGraph(x * 50 + 7, y * 50 + 57, effect[effect_wait / 6], TRUE);
                    if (effect_wait <= 0) {
                        situation[y][x] = 0;
                    }
                }
                else if (situation[y][x] == 4) {
                    DrawGraph(x * 50 + 7, y * 50 + 57, sign, TRUE);
                }
                else if (board[y][x] == 1) {
    ...
    if (GetMouseInput() & MOUSE_INPUT_LEFT) {
        ...
        if (board[my / 50][mx / 50] == 0) {
            if (situation[my / 50][mx / 50] == 0) {
                situation[my / 50][mx / 50] = 4;
            }
            else if (situation[my / 50][mx / 50] == 4) {
